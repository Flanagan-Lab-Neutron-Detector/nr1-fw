...
extern void	    QSPI_WriteBlock(uint32_t base, uint32_t count, void *block);
extern void     QSPI_ProgramWord(uint32_t Address, uint16_t word);
extern void	    QSPI_ProgramBuffer(uint32_t SectorAddress, void *data, uint16_t count);
extern void	    QSPI_ProgramBuffer_single(uint32_t SectorAddress, uint16_t word, uint16_t count);
